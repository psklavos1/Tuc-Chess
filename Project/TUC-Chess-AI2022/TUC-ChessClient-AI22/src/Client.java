	...
	private void receiveMessages() {
		...
		while (true) {
			try {
				...
				// no need to check for IPAddress and Port of sender, it must be the server of
				// TUC-CHESS
				...
				} else if (firstLetter.equals("G")) // received information is about the game (begin/end)
				{
					...
					if (secondLetter.equals("B")) {
						...
						if (myColor == 0) {
//							String action = world.selectActionMinimax();
							String action = world.selectActionAlphaBeta();
//							String action = world.selectRandom();
//							String action = world.selectActionMCTS();
					...
					{
						scoreWhite = Integer.parseInt(
								Character.toString(receivedMsg.charAt(2)) + Character.toString(receivedMsg.charAt(3)));

						scoreBlack = Integer.parseInt(
								Character.toString(receivedMsg.charAt(4)) + Character.toString(receivedMsg.charAt(5)));
				...
				} else // firstLetter.equals("T") - a move has been made
				{
					...
					scoreWhite = Integer.parseInt(
							Character.toString(receivedMsg.charAt(8)) + Character.toString(receivedMsg.charAt(9)));

					scoreBlack = Integer.parseInt(
							Character.toString(receivedMsg.charAt(10)) + Character.toString(receivedMsg.charAt(11)));
					
					updateScore(scoreWhite, scoreBlack);
...
//						String action = world.selectActionMCTS();
	...
	public void updateScore(int scoreWhite, int scoreBlack) {
		this.world.setScoreBlack(scoreBlack);
		this.world.setScoreWhite(scoreWhite);
	}
